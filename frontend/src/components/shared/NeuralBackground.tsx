...
export default function NeuralBackground({
  className,
  color = "#5D5FEF", // Default Indigo to match theme
  trailOpacity = 0.05, // Slower fade for smoother tails
  particleCount = 500, // Slightly fewer for clean look
  ...
  useEffect(() => {
    ...
    const init = () => {
      ...
      // Start with a white background so mix-blend-multiply works correctly
      ctx.fillStyle = 'white';
    ...
    // --- ANIMATION LOOP ---
    const animate = () => {
      // Fade to white for mix-blend-multiply on light theme
  ...
  return (
    <div ref={containerRef} className={cn("absolute inset-0 w-full h-full pointer-events-none -z-10", className)}>
      {/* We use mix-blend-multiply so the white trails become transparent over the mesh background */}
      <canvas ref={canvasRef} className="block w-full h-full mix-blend-multiply opacity-70" />
    </div>
  );
}
